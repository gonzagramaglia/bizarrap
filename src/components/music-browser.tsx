      ...
      >
        {parts.map((part, i) =>
          part.toLowerCase() === searchTerm.toLowerCase() ? (
            <span
              key={i}
              className="bg-yellow-300 dark:bg-yellow-400 text-black rounded-sm px-1 not-italic font-bold shadow-sm"
            >
              {part}
            </span>
          ) : (
    ...
    <div className="space-y-4 md:space-y-8 overflow-x-clip">
      <div className="flex flex-col md:flex-row items-center justify-center pt-0 pb-0 md:pt-0 md:pb-0 gap-0 md:gap-1 max-w-4xl mx-auto">
        ...
        <div className="flex flex-col items-center md:items-start md:gap-0">
          <h1 className="mx-auto md:mx-0 md:max-w-xl text-3xl md:text-5xl font-extrabold text-center md:text-left text-neutral-900 dark:text-white leading-tight tracking-tight">
            <>
              <span
                className={`bg-clip-text text-transparent bg-gradient-to-r ${theme === "dark" ? "from-white to-cyan-200" : "from-neutral-900 to-neutral-600"} block whitespace-pre-line drop-shadow-sm`}
              >
                {t("headline_part1")}
              </span>
              <span
      ...
      <div className="sticky top-0 z-50 py-2 md:py-3 pointer-events-none">
        ...
        >
          ...
          >
            ...
            <ArrowLeftRight
              className={`w-4 h-4 ${theme === "dark" ? "text-white" : "text-black"} group-hover:text-[#FACD00] transition-all duration-300 ${sortOrder === "desc" ? "rotate-180" : ""}`}
            />
            <span className="text-sm font-bold">
              {sortOrder === "asc" ? "66" : "00"}
            </span>
      ...
      >
        ...
        {filteredVideos.map((video, index) => {
          ...
          const localizedVideoName =
            language === "en"
              ? video.name.replace(/pr[oó]ximamente/gi, "COMING SOON")
              : video.name;
          return (
            <div
              key={video.id}
